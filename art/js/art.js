...
$('#CP .small_title').click(function () {
  let target = $(this).attr("for");
  $("#CP ." + target).slideToggle("fast");
});
/*++ready document++*/
...
function return_for_memory_value (e) {
  ...
  if ($('#draw_art').prop('checked')) {
    let data = document.getElementById("draw_art_canvas").getContext("2d").getImageData(0, 0, dac.width, dac.height);
...
function memory_value_into_canvas (key, name) {
  ...
  if (value.canvas === 'draw_art' && $('#draw_art').prop('checked')) {
    document.getElementById("draw_art_canvas").getContext("2d").putImageData(value.data, 0, 0);
...
//new_block_imgs
$('#CP .add_new_blocks button').click((e) => {
  $('#new_block_img').click();
});
...
function question_obj(x,y) {
  ...
  if (click_id === 'download_memory') {
    if ($('header .header_form p.language').text() === 'Japanese') {
      str = "cookieの有効期限を超えそうな時にダウンロードして下さい。"
      + "<br>「下書き」のメモリはダウンロード出来ません。"
      + "<br>「設計図」をダウンロードして次回の作業に使用して下さい。";
    }
    if ($('header .header_form p.language').text() === '英語') {
      str = "Download before the cookie deadline is about to exceeded."
      + '<br>"Draw art" memories cannot be downloaded.'
      + '<br>Download the "blueprint" and use it for your next job.';
    }
  }
...
function save_cookie_data() {
  /*make cookievalue*/
  let get_memorys_html = $('#syncer-acdn-03').html();
  ...
  let cookievalue = get_memorys_html;
